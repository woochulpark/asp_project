...
<%
	...
	SQL = "select a.행사번호, convert(varchar(16),빈소도착,120) as 빈소도착, isnull(convert(varchar(16),별세일시,120),'') as 별세일시, 고인명, 고인성별, 고인연령, 사망사유, 장례형태, 장례진행종교, 장례식장코드, "
...
<div class="sub_wrap">
	...
	<form name="frm" method="post" action="progression_goin_ok.asp">
	<input type="hidden" id="p29" name="p29" value="<%=input22 %>" />
	<input type="hidden" id="code" name="code" value="<%=code %>" />

	<table class="form_ty">
		<caption>진행-고인정보</caption>
		<colgroup>
			...
			<tr>
				...
				<td class="bdr"><input type="text" id="p1" name="p1" value="<%=input2_1 %>" class="input_ty w100"  placeholder="2019.09.22" disabled></td>
				<td class="bdr">
					<select id="p2" name="p2" class="select_ty w100" disabled><%=op_hour %></select>					
				</td>
				<td>
					<select id="p3" name="p3" class="select_ty w100" disabled><%=op_min_5 %></select>					
				</td>
			</tr>
			<tr>
				<th scope="row">별세일시</th>
				<td class="bdr"><input type="text" id="p4" name="p4" value="<%=input3_1 %>" class="datepicker input_ty start-date w100" placeholder="별세일시" disabled ></td>
				<td class="bdr">
					<select id="p5" name="p5" class="select_ty w100" disabled><%=op_hour %></select>
				</td>
				<td>
					<select id="p6" name="p6" class="select_ty w100" disabled><%=op_min_5 %></select>
				</td>
			</tr>
			<tr>
				<th scope="row">고인명</th>
				<td colspan="3"><input type="text" id="p7" name="p7" value="<%=input4 %>" class="input_ty w100" placeholder="" disabled></td>
			</tr>
			<tr>
				<th scope="row">고인성별/연령</th>
				<td class="bdr">
					<select id="p8" name="p8" class="select_ty w100" disabled>
						<option value="남">남</option>
						<option value="여">여</option>
					</select>					
				</td>
				<td colspan="2"><input type="text" id="p9" name="p9" value="<%=input6 %>" class="input_ty w90" disabled>세</td>
			</tr>
			<tr>
				<th scope="row">사망사유</th>
				<td colspan="3">
					<select id="p10" name="p10" class="select_ty w100" disabled>
			...
			<tr>
				...
				<td colspan="3"><input type="text" id="p12" name="p12" value="<%=input9 %>" class="input_ty w100" disabled></td>
			</tr>
			<tr>
				<th scope="row">장례식장</th>
				<td colspan="3">
					<input type="text" id="p13" name="p13" value="<%=input11 %>" class="input_ty w100" disabled>
			...
			<tr>
				<th scope="row" class="btnu">1차 장지<span class="th_str">(화장일 경우)</span><a href="#" class="btn_ico ico01" onclick="FirstJangJi();">1차 장지</a></th>
				<td colspan="3">
					<input type="text" id="p16" name="p16" value="<%=input13 %>" class="input_ty w100">
...
<%	if erpBtn <> "완료승인확인" then %>
	<div class="btm_btns sort03">
		<a href="javascript:void(0);" onclick="SaveERP();" class="btn_ty btn_b ty05">ERP 반영</a>
	</div>
<%	end if %>
...
<script language="javascript" type="text/javascript">
	...
	function FirstJangJi(sValue) {
		...
		$.ajax({
			...
			url: "/reception/reception_fristjangji_ajax.asp", //요청을 보낼 서버의 URL
	...
	function Save() {
		...
		if (!$("input:radio[name='p19']:checked").val()) {
			alert('버스사용 여부를 선택해 주세요.');
			return false;
		}
		if ($("input:radio[name='p19']:checked").val() == "Y" && !$("input:radio[name='p20']:checked").val()) {
			alert('버스사용 화장장 또는 장지를 선택해 주세요.');
			return false;
		}
		if (!$("input:radio[name='p21']:checked").val()) {
			alert('리무진사용 여부를 선택해 주세요.');
			return false;
		}
		if ($("input:radio[name='p21']:checked").val() == "Y" && !$("input:radio[name='p22']:checked").val()) {
			alert('리무진사용 화장장 또는 장지를 선택해 주세요.');
			return false;
		}
		if (!confirm("저장하시겠습니까?")) {
			return false;
	...
	function SaveERP() {
		...
		$('#p7').attr('disabled', false);
		$('#p8').attr('disabled', false);
		$('#p9').attr('disabled', false);
		$('#p10').attr('disabled', false);
		$('#p12').attr('disabled', false);
		$('#p13').attr('disabled', false);
				
		document.frm.action = "progression_goin_erp_ok.asp";
		document.frm.submit();
