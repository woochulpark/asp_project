...
<%
	...
	SQL = " select case when isnull(승인구분,'미승인') = '미승인' then '미승인' else 승인구분 end as 승인 "
	...
	if input1 = "승인" then
		...
		response.write "alert('보고서 승인 이후 현금영수증발행 불가');"	
		response.write "location.replace('/progression/progression_progress.asp?Code="& code &"');"
...
<div class="sub_wrap">
	...
	<input type="hidden" id="life_card_return"   	name="life_card_return"		value="<%=protocol%>hs.apluslife.co.kr/progression/progression_cash_hy.asp">	
	<input type="hidden" id="click_count"       	name="click_count"			value="0">

	<table class="form_ty">
		<caption>진행-배송외-고인정보</caption>
		<colgroup>
