...
<%	end if %>
	...
	<input type="hidden" name="tcenter" id="tcenter" value="<%=input24 %>" />

	<table class="table_ty verti">
		<caption>접수-배송-기본정보</caption>
		<colgroup>
		...
		<tbody>
			...
			<tr>
				<th scope="row" rowspan="2">담당 <br class="m_br">의전팀장<a href="#" class="btn_ico ico01 ly_open" onclick="TeamList();">의전팀장 배정</a></th>
				<td colspan="2">
					<span id="tname"><%=input5 %></span> 
					<a href="#" class="btn_ty ty02" onclick="TeamUpdate();">반영</a>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<th scope="row">직원명<!--a href="#" class="btn_ico ico03" onclick="MemberWrite();">직원명/연락처 수정</a--></th>
				<td colspan="2">
					<span id="mname_t"><%=input11 %></span> 
					<!--a href="#" class="btn_ty ty02" onclick="MemberUpdate();">반영</a-->
				</td>
			</tr>
			<tr>
				<th scope="row">연락처<!--a href="#" class="btn_ico ico04" onclick="alert('추후구현');">문자발송</a--></th>
...
<script language="javascript" type="text/javascript">
	...
	function MemberUpdate() {
		...
		$.ajax({
			...
			data: { Code: <%=code %>, mname: mname, mphone: mphone }, //서버로 보내지는 데이터
			datatype: "html", //응답 결과의 형태 (xml,html,script,json,jsonp,text)
			success: function (data, textStatus) { //응답객체 data:응답 결과, textStatus:성공여부 (성공일 경우만 success 문자열로 입력)
				alert('반영 되었습니다.');
			}
