...
<div class="sub_wrap">
	...
	<div class="top_btns sort01">
		<a href="javascript:void(0);" class="btn_ty ty02 btn_b" onclick="Save();">현금영수증 발행</a>
	</div>

	<form name="frm" method="post" target="card_frame" action="<%=protocol%>card.apluslife.co.kr/web/easypay_request_utf.asp">	
	<input type="hidden" id="code" name="code" value="<%=FnAesEncrypt(code, AesEncryptPwd) %>" />
	...
	<input type="hidden" id="EP_sub_mall_yn"       name="EP_sub_mall_yn"      value="0">           <!-- 하위 가맹점 사용여부 -->

	<!-- [END]가맹점 주문자 필드 -->

	<!-- [START]인증 요청 필드 -->
	<input type="hidden" id="EP_tr_cd"              name="EP_tr_cd"             value="00201050">   <!-- 거래구분(수정불가) -->
	<input type="hidden" id="EP_pay_type"           name="EP_pay_type"          value="cash">       <!-- 결제수단(수정불가) -->
	<input type="hidden" id="EP_req_type"           name="EP_req_type"          value="issue">      <!-- 카드결제종류(수정불가) -->

	<input type="hidden" id="EP_tot_amt"            name="EP_tot_amt"           value="0">           <!-- 결제총금액 -->
	<input type="hidden" id="EP_service_amt"        name="EP_service_amt"       value="0">
	<input type="hidden" id="EP_vat"				name="EP_vat"				value="0">
	...
	<input type="hidden" id="life_card_return"   	name="life_card_return"		value="<%=protocol%>hs.apluslife.co.kr/reception/reception_cash_b_hy.asp">	
	...
	<table class="form_ty">
		...
		<colgroup>
			...
			<tr>
				...
				<td colspan="4">
					<select id="self_issue" name="self_issue" class="select_ty w100" onchange="self_Change(this.value);">
						<option value="1" selected >고객요청(현금영수증)</option>
						<option value="2" >고객불필요(자진발급)</option>
					</select>					
				</td>
			</tr>
		    <tr>
				<th scope="row">영수증 수신번호</th>
				<td class="bdr" colspan="4"><input type="text" id="receipt_hp" name="receipt_hp" value="" class="input_ty w100" maxlength="12" placeholder="휴대폰번호 필수입력 미입력시 영수증 발송 불가(자진발급시 미입력)"></td>				
			</tr>
			<tr>
				...
				<td colspan="4">
					<select id="EP_product_nm" name="EP_product_nm" class="select_ty w100" onchange="amt_type_Change(this.value);">
						...
						<option value="추가매출">R/T결제</option>
					</select>					
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="4">
					<select id="EP_issue_type" name="EP_issue_type" class="select_ty w100">
			...
			<tr>
				...
				<td colspan="4">
					<select id="EP_auth_type" name="EP_auth_type" class="select_ty w100">
			...
			<tr>
				...
				<td class="bdr" colspan="4"><input type="text" id="EP_auth_value" name="EP_auth_value" value="" class="input_ty w100" maxlength="20" placeholder="소득: 주민번호(13자리)_휴대폰번호(-빼고입력), 지출: 사업자번호(10자리)"></td>				
			</tr>
...
<script language="javascript" type="text/javascript">
   ...
   /* 입력 자동 Setting */
    function f_init(){
      ...
      //amtChange();
		...
		%>
			...
			frm_pay.EP_auth_value.value   = "7805181463110";                                 //주민번호
			...
			frm_pay.EP_auth_type.value   = "02"; 
			frm_pay.receipt_hp.value   = "01033001620"; 
	...
	function self_Change(input) {

		if (input == "2" )
		{			
			document.getElementById("receipt_hp").value = "";
			document.getElementById("EP_issue_type").value = "01";
			document.getElementById("EP_auth_type").value = "03";
			document.getElementById("EP_auth_value").value = "0100001234";	

		}
		else {
			document.getElementById("EP_auth_value").value = "";	
		}		
	...
	function amtChange() {
		...
		if (ClearComma(document.getElementById("EP_product_amt").value) == "" || ClearComma(document.getElementById("EP_product_amt").value) <= 0 ) {
			...
			if (obj.value == "1" ){
				...
				document.getElementById("com_tax_amt").value = ClearComma(document.getElementById("com_tax_amt").value).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
				document.getElementById("com_vat_amt").value = ClearComma(document.getElementById("com_vat_amt").value).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");	
				...
				document.getElementById("com_free_amt").value = ClearComma(document.getElementById("EP_product_amt").value).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
				//alert("11");
				//return;
			}
	...
	function Save() {
		...
		*/
		...
		if ( $("#self_issue option:selected").val() == "01" && $("#receipt_hp").val().length < 10 ) {
		...
		*/
		if ( $("#EP_auth_type option:selected").val() == "" ) 
		{
			alert('인증구분 선택되지 않았습니다.');
			return false;
		}

		if ( $("#EP_issue_type option:selected").val() == "01" ) 
		{
			if ( $("#EP_auth_type option:selected").val() == "02" && $("#EP_auth_value").val().length < 13 ) 
			{
				alert('주민번호 입력되지 않았습니다. 13자리 입력입니다.');
				return false;
			}
			if ( $("#EP_auth_type option:selected").val() == "03" && $("#EP_auth_value").val().length < 10 ) 
			{
				alert('휴대폰번호가 입력되지 않았습니다. 10자리 이상입니다.');
				return false;
			}
			if ( $("#EP_auth_type option:selected").val() == "04" ) 
			{
				alert('소득공제용은 사업자번호를 선택할 수 없습니다.');
				return false;
			}
		}
		else {
			if ( $("#EP_auth_type option:selected").val() == "04" && $("#EP_auth_value").val().length < 10 ) 
			{
				alert('사업자번호가 입력되지 않았습니다. 10자리 입력입니다.');
				return false;
			}
			if ( $("#EP_auth_type option:selected").val() == "02" || $("#EP_auth_type option:selected").val() == "03"  ) 
			{
				alert('지출증빙용은 사업자번호만 선택하여야 합니다.');
				return false;
			}
		}
		...
		document.getElementById("EP_vat").value = ClearComma($("#com_vat_amt").val());
