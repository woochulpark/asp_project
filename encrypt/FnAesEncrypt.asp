...
' encrypted = FnAesEncrypt(dataIn, password)
 Function FnAesEncrypt(ByVal p_data, ByVal p_pwd)
  FnAesEncrypt = ""

  If Trim(p_data) = "" Or Trim(p_pwd) = "" Then
   Exit Function
  End If

  Dim objAes, encrypted

  Set objAes = New ClsAes

  encrypted = objAes.AESEncrypt(escape(p_data), p_pwd)

  Set objAes = Nothing

  FnAesEncrypt = encrypted
 End Function

 ' decrypted = FnAesDecrypt(encrypted, password)
 Function FnAesDecrypt(ByVal p_data, ByVal p_pwd)
  FnAesDecrypt = ""
  ...
  Dim objAes, decrypted

  Set objAes = New ClsAes

  decrypted = unescape(objAes.AESDecrypt(p_data, p_pwd))

  Set objAes = Nothing

  FnAesDecrypt = decrypted
 End Function
%>